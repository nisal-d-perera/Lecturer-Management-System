                ...
                <div class="container-fluid">
                    ...
                    <div class="row justify-content-center">
                        ...
                        <div class="col-lg-6">
                                ...
                                <div class="card mb-4">
                                    <div class="card-header">Update My Profile [<?=$admin_id?>] </div>
                                    <div class="card-body">
                                        <form method="POST">
               
                                            
                                            <div class="mb-3">
                                                <label class="small mb-1" for="admin_name">Admin Name</label>
                                                <input class="form-control" id="admin_name" name="admin_name" type="text" value="<?=$admin_name?>" placeholder="Admin Name" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="small mb-1" for="email">Email</label>
                                                <input class="form-control" id="email" name="email" type="email" value="<?=$admin_mail?>" placeholder="Email" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="small mb-1" for="nic">NIC Number</label>
                                                <input class="form-control" id="nic" name="nic" type="text" value="<?=$admin_nic?>" placeholder="NIC Number" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="small mb-1" for="start_date">Start Date</label>
                                                <input class="form-control" id="start_date" name="start_date" type="date" value="<?=$admin_sd?>" placeholder="Start Date" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="small mb-1" for="phone_number">Phone Number</label>
                                                <input class="form-control" id="phone_number" name="phone_number" type="text" value="<?=$admin_phone?>" placeholder="Phone Number" required>
                                            </div>
                                            <div class="mb-3">
...
if (isset($_POST['submit'])) {
    ...
    $admin_nic=$_POST['nic'];
    $admin_sd=$_POST['start_date'];
    $admin_phone=$_POST['phone_number'];
    $admin_username=$_POST['user_name'];
    
    //$id=$_GET['id'];

    $sql="UPDATE `admin` SET `admin_name`='$admin_name',`email`='$admin_email',`nic`='$admin_nic',`start_date`='$admin_sd',`phone_number`='$admin_phone' WHERE admin.admin_id = '$admin_id';";
    $sql="UPDATE `user` SET `user_name`='$admin_username' WHERE user.user_id = '$admin_user';";
    $conn->query($sql);
